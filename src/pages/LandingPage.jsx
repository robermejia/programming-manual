...
import { LogOut, User as UserIcon, CheckCircle, ChevronDown, ChevronUp } from 'lucide-react';

// Import all categories to calculate total topics
...
const LandingPage = () => {
  ...
  const getProgress = (manualId, categories) => {
    if (!categories || !userProgress) return 0;
    const allTopics = categories.flatMap(cat => cat.topics || []);
    if (allTopics.length === 0) return 0;
    const completed = userProgress[manualId] || [];
    return Math.round((completed.length / allTopics.length) * 100);
  };



  const manualStats = useMemo(() => [
    { id: 'js', categories: jsCategories },
    { id: 'ts', categories: tsCategories },
    { id: 'react', categories: reactCategories },
    { id: 'java', categories: javaCategories },
    { id: 'springboot', categories: springCategories },
    { id: 'angular', categories: angularCategories },
  ...
  return (
    <div className="landing-page">
      ...
      <header className="landing-header">
        ...
        <p>Tu referencia interactiva para el desarrollo moderno</p>
      </header>
      ...
      >
        ...
        <Link to="/js" className="manual-card js">
          ...
          <p className="card-desc">Sintaxis moderna y estructuras fundamentales.</p>
          <ProgressIndicator percentage={getProgress('js', jsCategories)} color="#F0DB4F" />
        </Link>

        {/* TypeScript Card */}
        <Link to="/ts" className="manual-card ts">
          <div className="card-icon">
            <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/typescript/typescript-original.svg" alt="TypeScript" width="40" height="40" />
          </div>
          <h2 className="card-title">TypeScript</h2>
          <p className="card-desc">Tipado estático, interfaces y genéricos.</p>
          <ProgressIndicator percentage={getProgress('ts', tsCategories)} color="#3178C6" />
        </Link>
        
        {/* React Card */}
        <Link to="/react" className="manual-card react">
          <div className="card-icon">
            <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/react/react-original.svg" alt="React" width="40" height="40" />
          </div>
          <h2 className="card-title">React 19</h2>
          <p className="card-desc">Hooks, Componentes y Server Actions.</p>
          <ProgressIndicator percentage={getProgress('react', reactCategories)} color="#61DAFB" />
        </Link>

        {/* Java Card */}
        <Link to="/java" className="manual-card java">
          <div className="card-icon">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128" width="40" height="40">
      ...
      >
        {gridExpanded ? (
          ...
          <><ChevronDown size={18} /> Ver todos los manuales</>
        )}
      </button>

      <footer className="landing-footer">
        <div className="footer-content">
          <p>&copy; {new Date().getFullYear()} Manual de Programación. Todos los derechos reservados.</p>
          <div className="footer-info">
            <span>Roberto Agustín Mejía Collazos</span>
...
const ProgressIndicator = ({ percentage, color }) => {
  ...
  return (
    <div className="progress-indicator">
      <div className="progress-text">
        ...
        <span style={{ color: isFinished ? '#10b981' : '#fff' }}>{percentage}%</span>
      </div>
      <div className="progress-bar-bg">
        <div 
          ...
          style={{ 
            width: `${percentage}%`, 
            background: isFinished ? '#10b981' : color 
          }} 
        />
      </div>
    </div>
